...
var tabbers = tabbers || (function() {
    ...
    var F = function() {
            ...
            let tabHolder = document.querySelectorAll(".rtabs li")
            ...
            tabHolder.forEach(tab => {
                tab.addEventListener("click", () => {
                    section = tab.getAttribute("section").trim()
                    console.log(section)
                })
            })
            ...
            updateInfoBtn.forEach(btn => {
                btn.addEventListener("click", () => {
                    ...
                    console.log(document.querySelector("#kids_category"))

                    console.log(section)

                    if (section === "Kids") {

                        document.querySelector("#category_updated").innerHTML = document.querySelector("#kids_category").innerText.trim()
                    } else if (section === "Women") {
                        document.querySelector("#category_updated").innerHTML = document.querySelector("#women_category").innerText.trim()
                    } else if (section === "Materials") {
                        document.querySelector("#category_updated").innerHTML = document.querySelector("#material_category").innerText.trim()
            ...
            updateImageBtn.forEach(btn => {
                btn.addEventListener("click", function() {
                    productId = btn.getAttribute("productId");
                    let images = this.parentElement.previousElementSibling.querySelectorAll("li");
                    imageUpdateModal.querySelector(".other-images").innerHTML = ""
                    images.forEach(img => {
                        imageUpdateModal.querySelector(".other-images").innerHTML += `
                        <div class="row item">
                            <div class=" col-12">
                                <img src="${img.innerText.trim()}" alt="" width="50%" class="img-fluid">
                            </div>
                            <div class="image-tools">
                                <button class="btn-danger btn delete-btn" image_id="${img.getAttribute("image_id")}">Delete Image <i class="fa fa-trash"></i></button>
                            </div>
                        </div>
                        `
                    })
            ...
            function more_images_for_product() {
                ...
                if (fileList.length > 3) {
                    ...
                    if (check) {
                        ...
                        fetch("/admin/add/3iuehohf98oIYYLYFKJ829", {
                                method: "post",
                                credentials: "include",
                                body: data
                            })
            ...
            function delete_image() {
                ...
                fetch(`/admin/del/${productId}/${imageId}`, {
                        method: "post",
                        credentials: "include"
                    })
                    .then(response => {
            ...
            function delete_product() {
                ...
                fetch(`/admin/del/3iuehohf98oIYYLYFKJ8297/8hd72/${productId}`, {
                        method: "post",
                        credentials: "include"
                    })
                    .then(response => {
            ...
            function update_product_info() {
                ...
                data['size'] = document.getElementById("productSize_updated").value.trim()
                data["price"] = document.getElementById("product_price_updated").value.trim()
                data["slashed_price"] = document.getElementById("slashed_price_updated").value.trim()
                ...
                data["isAvailable"] = document.getElementById("updated_available").value.trim()
            ...
            function add_product_function() {
                ...
                if (fileList.length > 6) {
                    ...
                    if (check) {
                        ...
                        data.append('size', document.getElementById("productSize").value.trim())
                        data.append("price", document.getElementById("product_price").value.trim())
                        data.append("slashed_price", document.getElementById("slashed_price").value.trim())
                        data.append("category", document.getElementById("category").value.trim())
                        data.append("description", document.getElementById("product_description").value.trim())
                        data.append("isAvailable", document.getElementById("available").value.trim())
                        data.append("root_category", section)
    ...
    t.prototype = {
        ...
        m: function(c, b) {
            var a = this;
            clearTimeout(a.c);
            a.c = setTimeout(function() { a.w(b) }, delay);
            return false
        },
        n: function(b, a) {
            u(b);
        ...
        o: function() {
            ...
            for (var f, d = 0; d < b.a[a]; d++) {
                ...
                if (f) {
                    if (!b.b) b.b = f[e];
                    if (x < 7) f[g].padding = "26px";
                    b.f.push(f);
                    if (this.P.b == "mouseover") {
                        b.a[d].onmouseover = function(a) { b.m(a, this) };
                        b.a[d].onmouseout = function() { clearTimeout(b.c) };
                        b.a[d].onclick = u
                    } else b.a[d].onclick = function(a) { b.n(a, this) }
                }
            }
        },
        p: function(a) {
            ...
            this.P = { a: a.license, b: a.event, c: a.autoAdvance, d: function() { typeof onTabSelected === "function" && onTabSelected(arguments[0], arguments[1]) } }
        },
        q: function(l) {
            var e = this;
            e.a = [];
            for (var j = l[h]("a"), c, b, f, d, g = 0; g < j[a]; g++) {
                b = j[g];
                f = b[k]("data-ajax");
                if (f) {
                    try { d = eval("(" + f + ")") } catch (m) { alert("data-ajax syntax error."); return }
                    d.cache = d.cache || 1;
                    b.b = d;
                    b.a = c = d.target
                } else {
        ...
        t: function(c) {
            function b(a, d) { var c = a.id; return c && d.indexOf(c) != -1 ? c : a[e].nodeName == "BODY" ? 0 : b(a[e], d) }
            var f = b(c, this.e);
            if (f) var g = this.r(f);
            if (g) {
                this.w(g);
                ...
                setTimeout(function() { window.scrollTo(d[0], a) }, 50)
            } else this.v()
        },
        v: function() {
            var b = this,
        ...
        w: function(h) {
            ...
            for (var j = 0, b = 0; b < this.a[a]; b++)
                if (d.a[b].a == h.a) {
                    v(d.a[b][e], "selected");
                    j = b
                } else r(d.a[b][e], "selected");
            for (b = 0; b < d.f[a]; b++)
                if (d.f[b].id == h.a) {
                    r(d.f[b], "inactive");
                    d.f[b][g].display = "block"
                } else {
                    v(d.f[b], "inactive");
        ...
        x: function(i, d, c) {
            ...
            j = d.id || 0;
            b.j = this.y();
            if (!l && x < 9) i = i + (i.indexOf("?") == -1 ? "?" : "&") + (new Date).getMilliseconds();
            b.j.open("GET", i, true);
                ...
                if (b.j && b.j.readyState == 4) {
                    if (b.h == c.id)
                        if (b.j.status == 200) {
                            ...
                            c[f] = m ? m(k) : q + "Failed to get data.";
                            b.i[c.id] = 0
                        }
                    b.j = null
                }
            };
            try { b.j.send(null) } catch (n) {}
        },
        y: function() { try { if (window.ActiveXObject) return new window.ActiveXObject("Microsoft.XMLHTTP") } catch (a) {} return new window.XMLHttpRequest },
        z: function() {
            var b = this;
            if (b.f[a] > 1) {
                b.b.onmouseover = function() { this.g = 1 };
    ...
    function z(f, e) {
        if (e) {
            if (typeof f[g].webkitAnimationName != "undefined") var b = "-webkit-";
            else if (typeof f[g].animationName != "undefined") b = "";
            else return;
            var h = "@" + b + "keyframes panelFadeIn {from{opacity:0;} to{opacity:1;}}",
                i = "div.panel-container > div {" + b + "animation: panelFadeIn " + e + "s;}";
    ...
    var D = function(d) {
            ...
            if (c[i]) c[i]("DOMContentLoaded", b, false);
            else w(window, "load", b)
        },
        o = function() {
            for (var e = c[h]("ul"), d = 0, i = e[a]; d < i; d++) j(e[d], "rtabs") && e[d][h]("a")[a] > 0 && b.push(new t(e[d], d));
            if (b[a]) {
                (new Function("a", "b", "c", "d", "e", "f", "g", function(c) { for (var b = [], a = 0, d = c.length; a < d; a++) b[b.length] = String.fromCharCode(c.charCodeAt(a) - 4); return b.join("") }("zev$nAjyrgxmsr,|0}-zev$eAjyrgxmsr,f-zev$gAf2glevGshiEx,4-2xsWxvmrk,-?vixyvr$g2wyfwxv,g2pirkxl15-?\u0081?vixyvr$|/e,}_4a-/e,}_6a-/}_5a?\u0081?zev$qAe2T2e\u0080\u0080+:+0rAtevwiMrx,q2glevEx,4--0vAQexl2verhsq,-0sA,hsgyqirx_h,+kvthpu+-a\u0080\u0080+pijx+-2vitpegi,g_r16a0f_r16a-2wtpmx,++-?mj,q%An,r/+9+0s--zev$wAh,+[hiz'[yphs']lyzpvu+-?mj,v@27-j_4a_iaAw?ipwi$mj,v@28-k_iaAw?\u0081vixyvr$e?"))).apply(this, [b[0], I, H, G, f, b[0].a, b[0].f[0]]).s();
                for (var g = 1, k = b[a]; g < k; g++) b[g].s();
